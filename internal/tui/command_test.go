...
func TestDisconnectCancelsBackgroundWork(t *testing.T) {
	...
	// A late conn-loss event after disconnect must not flip into a background
	// reconnect (we are no longer on the commander screen).
	m.reconnecting = false
	if cmd := m.beginReconnect(errClientClosed); cmd != nil {
		t.Fatal("beginReconnect after disconnect should be a no-op")
	}
	if m.reconnecting {
