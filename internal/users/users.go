...
var (
	...
	// ErrRevocationIncomplete — состояние пользователя ИЗМЕНЕНО и зафиксировано, но
	// отзыв по таблице §7.4 выполнен не полностью. §7.4 требует не возвращать успех
	// в этом случае, поэтому операция возвращает ошибку, хотя её первая половина
	// удалась.
	//
	// Восстановление — повтор той же команды: перевод в то же состояние разрешён
	// именно для этого и заново применяет таблицу §7.4. На границе server ошибка
	// отображается в INTERNAL: клиент ничего исправить не может, а оператор обязан
	// увидеть, что привилегии отозваны не до конца.
	ErrRevocationIncomplete = errors.New("users: state committed but revocation incomplete")
)
...
// Три порта §7.4 — Sessions, Tokens и Shares — различаются одним свойством:
// может ли отзыв не выполниться.
//
// Sessions и Tokens работают с состоянием ПРОЦЕССА. Реестр сессий закрывает
// сокеты, а transfer session token (§16.3) в §6 таблицы не имеет вовсе: сервер
// держит только хэш, токен привязан к control-сессии и живёт 900 секунд, то есть
// рестарт обесценивает его сам. Закрыть сокет и забыть значение из карты нельзя
// «наполовину», поэтому оба возвращают только число затронутых записей: ошибке
// взяться неоткуда, и вымышленный канал для неё заставил бы вызывающего писать
// обработку недостижимого случая.
//
// Shares — строки таблицы `shares` (§6.8), то есть SQLite: отменённый контекст,
// занятая база, ошибка ввода-вывода. Поэтому SetUserShares возвращает ошибку, и
// operation НЕ возвращает успех, пока ссылки не переведены (§7.4): иначе
// `user disable` сообщал бы об успехе, оставив публичные ссылки работающими, —
// именно тот случай, ради которого §7.4 требует приводить всё в соответствие ДО
// возврата успеха.
//
// Критерий именно такой — «персистентно ли состояние», — и он же указывает, что
// делать дальше: подсистема, переехавшая в БД, обязана сменить сигнатуру своего
// порта вместе с переездом.
...
type Sessions interface {
	...
	//
	// «ВСЕ» включает рукопожатия в полёте, и это требование к РЕАЛИЗАЦИИ, а не к
	// вызывающему (§7.4 п. 8). Соединение, успевшее пройти Authenticate против
	// старого stored_key, но ещё не попавшее в реестр, для наивного обхода
	// невидимо и стало бы сессией по старому паролю уже после успешного
	// `user passwd`. Реализация обязана сериализовать допуск и отзыв на одном
	// per-user примитиве и перепроверить под ним stored_key и состояние,
	// отказывая в допуске при расхождении.
...
type Tokens interface {
	...
	// Ошибки не возвращает: токен — состояние процесса (см. выше).
...
type Shares interface {
	...
	// число.
	//
	// Ошибка означает, что ссылки НЕ переведены, и операция обязана вернуть её
	// вместо успеха (§7.4). Реализация обязана быть идемпотентной: повторный
	// вызов после сбоя доводит перевод, а перевод в active касается только
	// ссылок в состоянии suspended — revoked терминально (§6.8, §17.4).
	SetUserShares(ctx context.Context, userID domain.UserID, state ShareState) (int, error)
}
...
func (noShares) SetUserShares(context.Context, domain.UserID, ShareState) (int, error) {
	return 0, nil
}
