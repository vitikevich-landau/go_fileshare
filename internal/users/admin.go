...
//
// assertActiveAdminRemains здесь НЕ вызывается, и это не пропуск. Create — чистый
// INSERT: множество активных администраторов он может только расширить, а
// инвариант 13 (§2.2, §7.4) нарушают ровно те четыре операции, которые способны
// его сузить, — `user disable`, `user delete`, `user purge` и понижение роли.
// §22 повторяет тот же перечень у кода LAST_ADMIN_REQUIRED = 117, так что
// проверка здесь выдавала бы код за операцию, которой он не положен. Случай «в БД
// вообще нет активного администратора» закрывается не отказом завести
// пользователя, а bootstrap safety §7.5: демон отказывается стартовать, первую
// запись создаёт --init-admin, а dev-лазейка называется --insecure-no-auth. Пока
// эта провязка не сделана (B1, §26), инвариант держится на старте, а не на
// `user add`.
