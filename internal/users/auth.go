...
// сейчас, потому что правила, которые он обязан соблюдать, относятся к M12–M13.
//
// AuthIters ОДИНАКОВ для всех логинов (§27 п. 2, §3.3). Возвращается действующее
// значение конфигурации, а не колонка строки: §6.2 п. 3 запрещает расхождение и
// между записями, и между записями и конфигом, оба случая отклоняются при старте
// (metadata.VerifyInvariants, Users.VerifyAuthIters), и брать значение из строки
// означало бы готовность обслуживать базу, которую старт обязан отвергнуть.
//
// Соль НЕСУЩЕСТВУЮЩЕГО логина — та же детерминированная `"fileshare-v2:" || login`,
// что и у существующего, и это прямое требование правила перехода §3.3 п. 1:
// фиктивная соль обязана иметь ТУ ЖЕ форму, в какой установка хранит настоящие,
// пока есть хотя бы одна запись с детерминированной солью. До M14 таких записей
// сто процентов — §6.2 п. 2 не разрешает никакой другой соли, — поэтому здесь
// ветка одна, и различить существование учётной записи по ответу невозможно
// вообще: соль вычисляется из логина, и это делает любой клиент сам.
//
// Формулы `HMAC-SHA256(server_secret, "auth-params-v1" || Login)[:16]` здесь
// сознательно НЕТ. Она вводится вместе со случайными солями (M14) и ровно тогда
// становится неотличимой от настоящей; введённая сейчас, она давала бы 16
// случайных байт там, где настоящая соль производна от логина, то есть сама
// отвечала бы на вопрос «есть ли такой пользователь» — в точности та ошибка,
// от которой §3.3 п. 1 защищает.
//
// Отсюда же и равенство работы, которого требует §3.3 п. 1: обе ветки выполняют
// один запрос и одну конкатенацию, и выравнивать нечего.
...
//
// РЕЗУЛЬТАТ ДЕЙСТВИТЕЛЕН НЕ ДОЛЬШЕ, ЧЕМ ДО ДОПУСКА СЕССИИ. Между чтением
// stored_key здесь и появлением сессии в реестре может пройти `user passwd`:
// SetPassword закоммитит новый ключ, CloseUser закроет зарегистрированные
// сессии и этого соединения не увидит, а доказательство, посчитанное против
// старого ключа, уже проверено. Допустить такую сессию значило бы завести вход
// по старому паролю ПОСЛЕ успешной смены. Поэтому §7.4 п. 8 обязывает
// реализацию Sessions сериализовать допуск и отзыв на одном per-user примитиве
// и перепроверить под ним stored_key и состояние. Сервису эту проверку сделать
// не за что: реестром владеет server, и провязка — первый коммит B1 (§26).
